...
import SmallWidthContainer from "../widgets/FullWidthContainer";
...
const SmallWidth = ({
  ...
  return (
    <div
      className={`hovered && ${"bg-white p-2 hover:outline my-2 hover:outline-1  hover:outline-blue-500 w-full flex justify-center"}`}
      ref={(ref: any) => connect(drag(ref))}
    >
      <div
        className={`w-[60%]     outline-2  border-2  ${size} mr-2  ${
          hovered && " "
      ...
      >
        {hovered && (
          <div>
            <div className="absolute  -bottom-4 -right-[30%] hover:bg-blue-500 text-white text-[10px] px-1 z-50 capitalize">
              {elementName}
            </div>
          </div>
        )}
