...
export default function TeamEventSetup({
  ...
  const [formValues, setFormValues] = useState<any>({
    ...
    widgetType: "",
    widgetShape: "square",
    appointmentTitle: calendar?.appointmentTitle,
  ...
  const handleSubmit = (e: any) => {
    ...
    if (!formValues.slug.trim()) {
      validationErrors.slug = "required";
    }
    // if (!formValues.widgetType.trim()) {
    //   validationErrors.widgetType = "required";
    // }
                  ...
                  <div key={index} className="tooltip" data-tip={item.title}>
                    <button
                      ...
                      className={`rounded-full w-8 h-8 ${item.color} ${
                        item.color === formValues.eventColor &&
                        "border-8 border-white-500 shadow"
                      }`}
                    ></button>
                  </div>
                ))}
