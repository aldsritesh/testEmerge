...
export default function ChatBody({
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        ...
        const response = await axios.get(
          `${baseUrl}messages?contactID=${lastContactId}${
    ...
    if (!isLastPage && lastContactId) {
      fetchMessages();
    }
  }, [fetchMessagesCount, lastContactId]);

  useEffect(() => {
    if (contact && lastContactId !== contact.id) {
      setIsLastPage(false);
      setLastMessageID("");
      setMessages([]);
      setLastContactId(contact.id);
      setFetchMessagesCount((prevCount) => prevCount + 1);
    }
  }, [contact]);
  ...
  return (
    <div className="flex flex-col h-[100%]">
      <div
        className={`w-full px-2 h-[90vh] overflow-y-scroll  scrollbar-hide ${
            ...
            <>
              <div className="flex flex-col" key={index}>
                <div className="flex justify-center m-4 sticky top-0 transition">
                  <span className="text-sm text-gray-600 py-0.5 border px-2 rounded-full bg-white border-gray-100 shadow-sm my-2">
                    {moment
                      .tz(message.date, "YYYY-MM-DD", "Etc/UTC")
                      .local()
                      .format("MMM DD, YYYY")}
                  </span>
                </div>
      ...
      {chatMailOpen && (
        <div
          className={`fixed border border-gray-300  bg-white w-full   flex pt-3 pb-2   overflow-y-scroll scrollbar-hide   ${
            chatMailOpen
            ...
            {conversationModeIndex === 0 ? (
              <form onSubmit={sendSMS}>
                ...
                <div className="justify-between flex mb-4 mx-2">
                  ...
                  <button
                    type="submit"
                    className="float-right  bg-[#1066cf] hover:bg-primary-hover focus:ring-1 focus:ring-primary  rounded-lg font-main text-sm focus:outline-none text-white font-semibold px-4 py-2 shadow-sm"
                  >
                    Send SMS
                  </button>
                </div>
              ...
              <form className="p-2 pb-4" onSubmit={sendEmail}>
                ...
                <div className="grid grid-cols-2">
                  ...
                  <div className="space-x-2 flex justify-end">
                    <div
                      className="cursor-pointer float-right bg-white border hover:bg-primary-hover focus:ring-1 focus:ring-primary  rounded-lg font-main text-sm focus:outline-none text-blue-700 font-semibold px-6 py-2 shadow-sm"
                    ...
                    <button
                      type="submit"
                      className="float-right  border border-blue-700 bg-[#1066cf] hover:bg-primary-hover focus:ring-1 focus:ring-primary  rounded-lg font-main text-sm focus:outline-none text-white font-semibold px-4 py-2 shadow-sm"
                    >
                      Send Email
                    </button>
                  </div>
