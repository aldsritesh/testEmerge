...
export interface ITextProps extends ICommonSettingsProps {
  ...
  text?: any;
  multitextInputProps?: InputHTMLAttributes<HTMLInputElement>;
  multitext?: any;
}

export const MultiboxTextInputElement = ({
  multitext,
  text,
  color = "#000000",
  multitextInputProps,
  ...
  const MultiboxHandleStoreData = (e: any) => {
  ...
  return (
    ...
    >
      ...
      <input
        ...
        {...multitextInputProps}
        placeholder={`${multitextInputProps?.placeholder} ${
          multitextInputProps?.required ? "*" : ""
        }`}
        onChange={(e) =>
          // setProp((props: any) => (props.textInputProps.value = e.target.value));
          MultiboxHandleStoreData(e)
        }
      />
    </div>
  );
...
MultiboxTextInputElement.craft = {
  related: {
