...
export default function EmailChartSetting({ ChangeSize }: any) {
  const dashctx: any = useContext(GlobalContext);

  const SizeRadio = [{ name: "Small" }, { name: "Medium" }, { name: "Large" }];

  const [emailData, setEmailData] = useState({
    emailHeader: dashctx?.formValues?.find(
      (item: { id: number }) => item?.id === 7
    )?.formData?.emailHeader,
    Source: dashctx?.formValues?.find((item: { id: number }) => item?.id === 7)
      ?.formData?.Source,
    Type: dashctx?.formValues?.find((item: { id: number }) => item?.id === 7)
      ?.formData?.Type,
    size: "",
  });
  ...
  const handleChange = (e: any) => {
    ...
    setEmailData((prevData: any) => ({ ...prevData, [name]: value }));
  ...
  const ValueSourceData = ["Sent", "Opened", "Clicked", "Sign Ups"];
  const ValueTypeData = [
    "1 day",
    "7 day",
    "14 day",
  ...
  return (
    <div className="bg-white w-[30%] px-4 py-2 h-[89vh] overflow-y-scroll scrollbar-hide">
      <p className="mb-4  text-lg font-semibold">
        {
          dashctx?.formValues?.find((item: { id: number }) => item?.id === 7)
            ?.name
        }
      </p>
