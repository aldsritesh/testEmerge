...
import { dashContext } from "./Maindashboard";
import Task from "../GridComponent/GridTask";
import PipelineSalesChart from "../GridComponent/SalesPipeline";
import DealsRevenueForecast from "../GridComponent/DealsRevenueForeCast";
const ResponsiveReactGridLayout = WidthProvider(Responsive);

function Grid({ SetBoxId }) {
  const defaultProps = {
    className: "layout",
    rowHeight: 55,
  ...
  useEffect(() => {
    setMounted(true);
    const prevewing = router?.query["preview"];
  ...
  const _makeSmall = (params) => {
    ...
    if (breakpoint === "lg") {
      ...
      if (tempComps[params].component === "calender")
        tempComps[params].origin.h = 5;
      if (tempComps[params].component === "task")
        tempComps[params].origin.h = 5;
      if (tempComps[params].component === "pipelineSales")
        tempComps[params].origin.h = 5;
    } else if (breakpoint === "md") {
      if (tempComps[params].component === "chat")
        tempComps[params].origin.h = 9;
        ...
        {comps?.map((c, i) => {
          return (
            <div
              ...
              key={i}
              onClick={(e) => _handleClick(e, i)}
              data-grid={{ ...c.origin, static: previewing || save }}
              className={`
                ${selectedIndex === i ? "border  border-blue-500 rounded" : ""}
              `}
            >

              {c.component === "numbers" ? (
                <Numbers index={i} size={c.origin.w} breakpoint={breakpoint} />
              ) : null}
              ...
              {c.component === "chat" ? (
                <DealsRevenueForecast breakpoint={breakpoint} size={c.origin.w} index={i} />
              ) : null}
              {c.component === "email" ? <Emailreport index={i} /> : null}
              {c.component === "gantt" ? <Gantt index={i} /> : null}
              {c.component === "task" ? <Task breakpoint={breakpoint} size={c.origin.w} index={i} /> : null}
              {c.component === "pipelineSales" ? <PipelineSalesChart index={i} /> : null}

              <div
                style={{ zIndex: 9 }}
                id={`di-${i}`}
                className={selectedIndex === i ? "" : "hidden"}
              >
                <div className="grid grid-cols-1 sm:grid-cols-3 w-56 rounded-md gap-4 mb-2 bg-white p-2 mt-2 ">
                  <div>
                    <div className="relative ">
                      <label className="flex  cursor-pointer rounded-md border border-grey/20 bg-white justify-between items-start gap-4 text-center">
                        <div className="h-6 w-4/12 bg-gray-200  rounded-md"></div>
                        <input
                  ...
                  <div>
                    <div className="relative">
                      <label className="flex cursor-pointer rounded-md border border-grey/20 bg-white justify-between items-start gap-4 text-center">
                        <div className="h-6 w-6/12 bg-gray-200 rounded-md"></div>
                        <input
